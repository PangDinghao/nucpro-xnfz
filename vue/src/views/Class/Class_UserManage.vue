      ...
      <template>
          <div style="margin: 20px;width: 100%;text-align: center">
            <div style="display: flex; align-items: center; justify-content: space-between;">
              ...
              <el-select v-model="AddStudentId" multiple filterable  placeholder="请选择学生">
                <el-option
                    v-for="user in noclassStudentDatea"
                    :key="user.userid"
                    :label="user.username"
                    :value="user.userid">
                </el-option>
              </el-select>
              <el-button type="success" style="margin-right: 30px" @click="add">添加班级成员</el-button>
            </div>
          </div>
      </template>
      ...
      <el-table :data="tableData" :show-header="false">
        ...
        <el-table-column width="50px" >
          <template slot-scope="scope">
            <el-popconfirm
                title="确定移除吗？"
                @confirm="removeOutClass(scope.row.userid)"
            >

      ...
      <template>
        <div style="margin: 20px;width: 100%;text-align: center">
          <div style="display: flex; align-items: center; justify-content: space-between;">
            ...
            <el-select v-model="AddClassId" multiple filterable  placeholder="请选择班级">
              <el-option
                  v-for="class1 in noplanClassDate"
                  :key="class1.class"
                  :label="class1.classno"
                  :value="class1.classid">
              </el-option>
            </el-select>
            <el-button type="success" style="margin-right: 30px" @click="add">添加实训班级</el-button>
          </div>
        </div>
      </template>
      ...
      <el-table :data="tableData" :show-header="false">
        ...
        <el-table-column width="50px">
          <template slot-scope="scope">
            <el-popconfirm
                title="确定移除吗？"
                @confirm="removeOutClass(scope.row.classid)"
            >
...
export default {
  ...
  methods:{
    ...
    add(){
      this.$axios.post(this.$httpUrl+'/class/addToPlan', {
        AddClassId : this.AddClassId,
    ...
    removeOutClass(classid){//将用户的classid置0（移出班级）
      const classIdArray = [classid];
      this.$axios.post(this.$httpUrl+'/class/removeToPlan', {
        AddClassId : classIdArray,
        planid : this.plan.planid ,
    ...
    ReturnToTrainingPlan_ClassManage(){
      this.$router.push("TrainingPlan_Manage");
    }
  },
  computed: {
    beforeMount() {
